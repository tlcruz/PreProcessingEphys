function GuiPlotRawDataReplayCurateSR(path)
...
CCT = uicontrol(f,'Style','edit','String','CC Lag',...
...
% Store the spike rate calculated using the defined transformations
    function PlotFly_Callback(source, eventdata)
        ...
        if currentFly <= length(flies)
            ...
            indMax = zeros(size(locs));
            wi = 15;
            for k = 1 : length(locs)
                if (locs(k) + wi < length(MPOri2)) && (locs(k) - wi > 0)
                    windw = MPOri2((locs(k) - wi):(locs(k) + wi));
                    [~,IMa] = max(windw);
                    [~,IMi] = max(-windw);
                    if(IMa > IMi)
                        indMax(k) = 1;
            ...
            w = (dt.params.SampRateDAC/pTransform.GaussWindow(1)) * gausswin(pTransform.GaussWindow(1));
    ...
    % Function that handles the high pass filter cutoff input
...
%% UpdateAxes
    function updateAxes()
        ...
        VisOut = dt.Data{currentPart}.VisRep;
...
%         plot(Tt, -20+SROut, 'k', 'linewidth', 0.5)
        ...
        [~, ActBoutsV] = GetBouts(VisOut, zeros(size(VfOut)), zeros(size(VsOut)), 1000, 500, 5, 0.1);
        cvsR = [];
        ...
        hold on        
        if ~isempty(cvsR)
            [~,IR] = max(abs(nanmean(cvsR,2)));
            plot([lagsM(IR) lagsM(IR)]/dt.params.SampRateTreadmill, [-1 1], '--b', 'linewidth', 0.5)
            plot(lagsM/dt.params.SampRateTreadmill, nanmean(cvsR,2), 'b', 'linewidth', 3)
            plot(lagsM/dt.params.SampRateTreadmill, nanmean(cvsR,2)+nanstd(cvsR,1,2), 'b', 'linewidth', 1)
            plot(lagsM/dt.params.SampRateTreadmill, nanmean(cvsR,2)-nanstd(cvsR,1,2), 'b', 'linewidth', 1)
            axis([-minL/(2*dt.params.SampRateTreadmill) minL/(2*dt.params.SampRateTreadmill) -1 1])
        end
        if ~isempty(cvsV)
            [~,IV] = max(abs(nanmean(cvsV,2)));
            ...
            plot(lagsV/dt.params.SampRateTreadmill, nanmean(cvsV,2), 'r', 'linewidth', 3)
            plot(lagsV/dt.params.SampRateTreadmill, nanmean(cvsV,2)+nanstd(cvsV,1,2), 'r', 'linewidth', 1)
            plot(lagsV/dt.params.SampRateTreadmill, nanmean(cvsV,2)-nanstd(cvsV,1,2), 'r', 'linewidth', 1)
            axis([-minL/(2*dt.params.SampRateTreadmill) minL/(2*dt.params.SampRateTreadmill) -1 1])
        end
    end
